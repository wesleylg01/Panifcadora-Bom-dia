...
totalcx := totalcx - totalsan;
 ...
 begin
  ShowMessage('ERRO: Valor Invalido, você não tem esse valor em Caixa !!');
  totalsan := 0;
 end;
...
begin
  ...
  begin
    if application.MessageBox('Deseja Realmente Sair ?' , 'Atenção' , mb_iconquestion + mb_yesno )= id_yes
     then
      Form_entracx.close;
    end
  else
  showmessage ('ERRO: Termine as transações para sair !!');
    end;
