...
begin
 ...
 usuario não consiga sair com a
 venda em andamento}
 if sair = 0 then
  begin
   if application.MessageBox('Deseja Fechar o Caixa ?' , 'Atenção' , mb_iconquestion + mb_yesno )= id_yes
   then Form_pdv.Close;
   end
  else
  begin
...
begin
  ...
  sair:= 0;
    ...
    then
     begin
      Memo1.Lines.Clear;
      Memo1.Lines.Add('=========== Bom dia Pães e Confeitaria ============');
      Edit_cod.Clear;
      Edit_qtd.Clear;
      Label_und.Caption := '0,00';
  ...
  if Edit_cod.Text <> '' then
    begin
      ...
      Edit_qtd.Clear;
        ...
        Memo1.Lines.Add('Preço und: ' + Floattostr (vlproduto)+ 'R$' );
    ...
    // verificando o estoque do produto
      if est > 0 then
        begin
         ...
         if est  <= 0 then
          begin
           application.MessageBox ('ERRO 004 - Não é possivel vender este produto, pois não temos ele em estoque !','ERRO 004',MB_ICONWARNING);
...
procedure TForm_pdv.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
//faz a função de dar o foco no edit_qtd para o usuario digitar a quantidade de determinado produto, pela tecla F1
 if key = vk_F1 then
   begin
     Edit_qtd.SetFocus;
   end;

//faz a função de sair da tela de pdv, pela tecla F4
 if key = vk_F1 then
   begin
     Edit_qtd.SetFocus;
   end;

//faz a função do botao "Finalizar venda", pela tecla F6
 if key = vk_F6 then
  begin
  sair:= 0;
  totaldincx:= totaldincx + vlgeral;
        ...
        vlgeral2:= vlgeral;
          ...
          Memo1.Lines.Add('=========================================');
           //Chamando o form de conclução de venda
 ...
 begin
//faz a função do botao "cancelar item", pela tecla F8
 if key = vk_F11 then
  cancelar;
 end;
