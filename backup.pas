...
begin
if application.MessageBox('Deseja Realmente fazer o Restore? Isso fará com que todas as alterações feitas no banco de dado/s depois ultimo backup se perca.' , 'Atenção' , mb_iconquestion + mb_yesno )= id_yes
then
ShellExecute(handle,'open', 'cmd.exe',Pchar('/c  C:\mysqldump -h localhost -u root panificadora < C:\Backup\backup.sql'),nil, SW_SHOW )
else
