...
begin
  ...
  ZQuery2.Open;
  cond:= '';
  if ((radio_ativo.Checked = false) and (radio_desativo.checked = false)) then
  begin
    application.MessageBox('001 - Campo vazio ao tentar realizar cadastro','ERRO 001',MB_ICONWARNING);
  end;
  ...
  begin
  application.messagebox('ERRO 008 - Nome de usuário ja cadastrados','ERRO 008',MB_ICONWARNING);
  edit3.setfocus;
  end
  else
  salvar;
...
ZQuery1.Close;
ZQuery1.SQL.Clear;
ZQuery1.SQL.Add('insert into USUARIOS (nome,cpf,login,senha,condicao)values(:nome, :cpf, :login, :senha, :condicao)');
...
ZQuery1.ParamByName('condicao').AsString:= cond;
ZQuery1.ExecSQL;
Edit1.Clear;
Edit2.clear;
Edit3.Clear;
Edit4.Clear;
Edit1.SetFocus;
radio_ativo.Checked := false;
radio_desativo.Checked := false;
end;
...
if Edit1.Text = '' then
 ...
 begin
  ...
  testar_duplicidade;
  Application.MessageBox('Usuário Cadastrado com sucesso','Usuário Cadastrado', MB_ICONINFORMATION)
  end;
